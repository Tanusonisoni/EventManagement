...
export async function updateEvent(req,res,next) {
    ...
    if(!event){
        res.status(404).json(new ApiResponse(false,null,"event not found"));

    }
    res.status(200).json(new ApiResponse(true,event,"update successfully"));
...
export async function deleteEvent(req,res,next) {
    try{
        ...
        res.status(200).json(new ApiResponse(true,event,"event deleted"));
...
export async function getAllEvent(req,res,next) {
    try{
        ...
        .populate("createdBy","name email role_id")
        .skip(skip)
        .limit(limit)
        .sort({createdAt:-1});

        let totalEvent=await eventModel.countDocuments();

        res.status(200).json(new ApiResponse(true,{page,limit,totalEvent},"success"));

    }catch(error)
    {
        res.staus(500).json(new ApiResponse(false,null,error.message||"internal sever error"))
    }
}

export async function cancelEvent(req,res,next) {
    try{
        let isCancelled=req.query.isCancelled === "ture" ? true:false;
        let event=await eventModel.findByIdAndUpdate(req.params.EventId,{isCancelled,isUpcoming:false},{returnDocument:"after"});

        if(!event)
        {
            return res.staus(404).json(new ApiResponse(false,null,"event not found"));
        }
        res.status(200).json(new ApiResponse(true,event,"cancel successfull"));
    }catch(error)
    {
        res.status(500).json(new ApiResponse(false,null,error.message || "internal server error"));
    }
...
export async function searchEvents(req, res, next) {
    try {
        let category = req.query.cate || "";
        let titleRegex = new RegExp(req.query.title, "i");
        ...
        res.status(200).json(new ApiResponse(true, events, "success"));
...
// filter events by cate , is upcoming , is ongoing , is completed , is cancelled
export async function filterEvents(req, res, next) {
    try {
        let category = req.query.cate || "";
        let isUpcoming = req.query.upcoming === "true";
        let isOngoing = req.query.ongoing === "true";
